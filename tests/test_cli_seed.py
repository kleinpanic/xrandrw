...
``_prev_present`` stays None; a later settle would then seed off an
...
class FakeCoordinator:
    """Seeds (_prev_present set) only if dwm-ipc is available at on_settled time."""
...
def test_seed_waits_then_seeds_when_dwmipc_comes_up(monkeypatch, logger):
    ...
    assert coord._prev_present is not None, "seeded once dwm-ipc became available"
...
def test_config_off_disables_relocation_even_with_dwmipc(monkeypatch, logger):
    ...
    # available: on_settled never seeds, so _prev_present stays None (no
    ...
    assert coord._prev_present is None, "config-off gates the feature off despite dwm-ipc"
